...
function pathclick(evt){
  var tag = evt.target, valor = "normal"; 
  ...
  if(estilo_mapa == 1){
    $.get("config/update_query.php", {aid: tag.id, classe: valor, func: "atualiza_cor_parada"});
  } else {$.get("config/update_query.php", {aid: tag.id, classe: valor, func: "atualiza_cor"}); }
